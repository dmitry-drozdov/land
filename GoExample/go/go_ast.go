...
func ParseFiles(root string) (map[string]map[string]*FuncStat, map[string]map[string]*StructStat, int, error) {
	...
	err := filepath.Walk(root, func(path string, info os.FileInfo, _ error) error {
		...
		g.Go(func() error {
			t0 := time.Now()
			nodesReq, nodesAll, f, dataFunc, dataStruct, err := ParseFile(pathBk)
			if err != nil {
				return err
			}
...
func ParseFile(path string) (int, int, *ast.File, map[string]*FuncStat, map[string]*StructStat, error) {
			...
			case *ast.StructType:
				...
				for _, f := range y.Fields.List {
					for i := 0; i < len(f.Names); i++ { // a, b, c int => append 3 int
						resStruct[x.Name.Name].Types = append(resStruct[x.Name.Name].Types, HumanType(f.Type, &reqCnt))
					}
...
func HumanType(tp ast.Expr, reqCnt *int) string {
	*reqCnt++
	switch t := tp.(type) {
	case *ast.Ident:
		return t.Name
	...
	case *ast.StarExpr:
		return HumanType(t.X, reqCnt)
	case *ast.ArrayType:
		return HumanType(t.Elt, reqCnt)
	case *ast.IndexExpr:
		return HumanType(t.X, reqCnt)
